...
gdot_values = [0.0, 0.001, 0.005, 0.01, 0.02]
gdot_labels = ["0", "0.001", "0.005", "0.01", "0.02"]
...
for fs in fs_values:
    for gdot in gdot_values:
        folder = find_case_folder(fs, gdot)
        dump_file = find_dump_file(folder)

        print(f"Reading fs={fs}, gdot={gdot}: {dump_file}")
        ...
        all_frame_records.append(last_df)
...
last20_frames_df = pd.concat(all_frame_records, ignore_index=True)

summary_df = summary_df.sort_values(["fs", "gdot"]).reset_index(drop=True)
last20_frames_df = last20_frames_df.sort_values(["fs", "gdot", "frame_index"]).reset_index(drop=True)

out_dir = root / "Processed_Data"
out_dir.mkdir(parents=True, exist_ok=True)

out_summary_csv = out_dir / "Fig6_shear_statistics_last20_summary.csv"
out_frames_csv = out_dir / "Fig6_shear_statistics_last20_frames.csv"

summary_df.to_csv(out_summary_csv, index=False, encoding="utf-8-sig")
last20_frames_df.to_csv(out_frames_csv, index=False, encoding="utf-8-sig")
